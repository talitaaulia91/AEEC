...
insert  into `batch_program`(`ID_BATCH`,`ID_PROGRAM`,`TGL_MULAI`,`TGL_BERAKHIR`,`BATCH`,`STATUS`) values 
('FFNF001','FFNF','2022-04-20','2022-04-23',1,'1');
...
CREATE TABLE `client` (
  ...
  `BERKAS_NPWP` varchar(255) NOT NULL,
...
insert  into `diskon`(`ID_DISKON`,`NAMA_DISKON`,`PRESENTASE`,`BENTUK`,`DESKRIPSI`) values 
('D01','voucher 5%',5,'voucher',NULL),
('D02','cashback 10% (mengajak 3 partisipan)',10,'cashback',NULL),
('D03','cashback 5% (follow sosmed AEEC)',5,'cashback','\r\n   \r\n ');
...
('FFNF','SR','FINANCE ACCOUNTING FOR NON-FINANCIAL MANAGER',2220000,1776000,1243200,'tes',7,20,'register (3).png');
...
insert  into `user`(`ID_USER`,`EMAIL`,`NAMA`,`PASSWORD`,`ROLE`,`CREATED_AT`) values 
('US04220001','administrator@gmail.com','','$2y$10$9tQTwh3ck7pHM1zpKaGb8u8go3Wtj1clERyKivi4yvfZglbkt4ozm','admin','2022-04-20');
...
BEGIN 
	SET NEW.ID_CLIENT = CONCAT('PS',DATE_FORMAT(SYSDATE(), "%m"),
...
BEGIN 
	SET NEW.ID_PEMBAYARAN = CONCAT('PAY',DATE_FORMAT(SYSDATE(), "%m"),
	CAST(SUBSTR(YEAR(SYSDATE()),3,4)AS INT),
	LPAD((IFNULL((SELECT CAST(SUBSTR(ID_PEMBAYARAN,8,3)AS INT) 
	FROM pembayaran
	WHERE  TGL_PEMBAYARAN = NEW.TGL_PEMBAYARAN
...
BEGIN 
	SET NEW.ID_PENDAFTARAN = CONCAT('REG',DATE_FORMAT(SYSDATE(), "%m"),
	CAST(SUBSTR(YEAR(SYSDATE()),3,4)AS INT),
	LPAD((IFNULL((SELECT CAST(SUBSTR(ID_PENDAFTARAN,8,3)AS INT) 
	FROM pendafatran
	WHERE  TGL_PENDAFTARAN = NEW.TGL_PENDAFTARAN
...
BEGIN 
	SET NEW.ID_USER = CONCAT('US',DATE_FORMAT(SYSDATE(), "%m"),
	CAST(SUBSTR(YEAR(SYSDATE()),3,4)AS INT),
